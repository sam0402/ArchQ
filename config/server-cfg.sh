...
case $server in
    ...
    M)
        if ! pacman -Q mpd-light >/dev/null 2>&1; then
            ...
            pacman -U --noconfirm /tmp/mpd-light-0.23.13-4-x86_64.pkg.tar.zst /tmp/mpd_cdrom-1.0.0-1-any.pkg.tar.zst /tmp/rompr-2.00-1-any.pkg.tar.zst /tmp/mympd-12.1.1-1-x86_64.pkg.tar.zst
